...
class VerticalTicker extends Component {
  constructor(props) {
    super(props);
    this.state = {
      ...
      moving: 0,
      moveInterval: null,
      items: (Array.isArray(this.props.children) && this.props.children) || [
        this.props.children,
      ],
    };
  ...
  pause() {
    if (!this.state.paused)
      this.setState({ paused: 1 }, () => this.props.paused());
  }
  unPause() {
    if (this.state.paused) {
  ...
  checkSpeed() {
    if (this.props.duration < this.state.speed + 25)
      this.setState((_, props) => ({ speed: props.duration - 25 }));
  }
